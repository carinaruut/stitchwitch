...
const cellSize = ref(savedPreferences.cellSize ?? Math.min(40, Math.max(18, pattern.project.value.cellSize)))
...
const completedColorCounts = computed(() => {
  ...
  pattern.cells.forEach((row, rowIndex) => row.forEach((color, columnIndex) => {
    if (!isStitchCompleted(pattern.cellIds[rowIndex][columnIndex], progress)) return
...
const confirmation = computed(() => resetModalOpen.value ? { title: t('tracker.confirm.reset.title'), message: t('tracker.confirm.reset.message'), label: t('tracker.confirm.reset.action') } : null)
...
function selectStitch(row: number, column: number) {
  ...
  state.selectStitch(row, column, renderedPattern.value.cellIds)
}

function selectStitches(cells: Array<[row: number, column: number]>, completed: boolean) {
  if (!renderedPattern.value) return
  state.selectStitches(cells, renderedPattern.value.cellIds, completed)
}
...
function confirmActiveModal() {
  if (resetModalOpen.value) confirmReset()
}
...
<template>
  <div class="space-y-4 text-base-content">
    <!-- Standalone Tracker navigation and file-open state were removed when Tracker became an Editor workspace.
      ...
      <section class="card border border-base-300 bg-base-100">
        <div class="card-body gap-4 p-4 sm:p-5">
            ...
            <div class="flex flex-wrap items-center justify-end gap-2">
              ...
              >
                <span
                  ...
                  aria-hidden="true"
                />
                {{ pattern.autosaveStatus.value === 'error' ? t('tracker.autosave.failed') : pattern.autosaveStatus.value === 'saving' ? t('tracker.autosave.saving') : t('tracker.autosave.saved') }}
              </span>
            </div>
          </div>
          <progress
          ...
          <div class="flex items-center justify-between text-xs text-base-content/60">
            <span>{{ t('tracker.progress.percentComplete', { percentage: n(percentage / 100, 'percent') }) }}</span><span>{{ t('tracker.progress.updated', { date: d(new Date(activeTracker.progress.updatedAt), 'short') }) }}</span>
          </div>
        </div>
      </section>
      ...
      <section class="card border border-base-300 bg-base-100">
        <div class="card-body gap-4 p-3 sm:p-5">
                ...
                <div class="flex flex-col gap-4">
                  <label class="form-control gap-2">
                    ...
                    <select
                      ...
                      :value="activeTracker.progress.completionMode"
                  ...
                  <label class="form-control gap-2">
                    ...
                    <select
                      class="select select-bordered select-sm w-full"
                      :value="activeTracker.progress.startRow"
                      :disabled="state.completedCount.value > 0"
                  ...
                  <label class="form-control gap-2">
                    ...
                    <select
                      class="select select-bordered select-sm w-full"
                      :value="activeTracker.progress.firstRowDirection"
                      :disabled="state.completedCount.value > 0"
                  ...
                  <label class="flex items-center justify-between gap-3 text-sm">
                    ...
                    <input
                      ...
                      type="checkbox"
                      :checked="activeTracker.progress.alternateRows"
                      :disabled="state.completedCount.value > 0"
          ...
          <p class="text-sm text-base-content/65">
            {{ t(addingComment ? 'tracker.comments.addInstruction' : activeTracker.progress.completionMode === 'individual' ? 'tracker.instructions.individualUsage' : 'tracker.instructions.usage') }}
          </p>
          ...
          <TrackerGrid
            ...
            @row="state.selectRow($event, renderedPattern.cellIds)"
