  ...
  <section class="flex flex-wrap items-center gap-2 rounded-box border border-base-300 bg-base-200/70 p-1.5" :aria-label="t('controls.drawing.regionLabel')">
      <div class="flex flex-wrap gap-1" :aria-label="t('controls.drawing.selectionLabel')">
        ...
        <div class="tooltip" :data-tip="t('controls.drawing.clearGrid')">
          <button class="btn btn-sm btn-ghost" type="button" :aria-label="t('controls.drawing.clearGrid')" @click="$emit('clear')"><span class="mdi mdi-delete-sweep-outline text-lg" aria-hidden="true"></span></button>
        </div>
      </div>
      <div class="flex gap-1" :aria-label="t('controls.drawing.mirrorLines')">
        <div class="tooltip" :data-tip="t('controls.drawing.verticalMirrorTooltip')">
          <button class="btn btn-sm" :class="mirrorVertical ? 'btn-secondary' : 'btn-ghost'" type="button" :aria-label="t('controls.drawing.toggleVerticalMirror')" :aria-pressed="mirrorVertical" @click="$emit('toggleMirrorVertical')"><span class="mdi mdi-flip-horizontal text-lg" aria-hidden="true"></span><span class="hidden xl:inline">{{ t('controls.drawing.vertical') }}</span></button>
        </div>
        <div class="tooltip" :data-tip="t('controls.drawing.horizontalMirrorTooltip')">
          <button class="btn btn-sm" :class="mirrorHorizontal ? 'btn-secondary' : 'btn-ghost'" type="button" :aria-label="t('controls.drawing.toggleHorizontalMirror')" :aria-pressed="mirrorHorizontal" @click="$emit('toggleMirrorHorizontal')"><span class="mdi mdi-flip-vertical text-lg" aria-hidden="true"></span><span class="hidden xl:inline">{{ t('controls.drawing.horizontal') }}</span></button>
        </div>
      </div>
