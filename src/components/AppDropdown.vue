...
const position = ref({ top: 0, left: 12, maxHeight: 320, above: false })
const panelId = `dropdown-${useId()}`
const isOpen = computed(() => props.open ?? internalOpen.value)
const panelClass = computed(() => [
  props.width !== 'auto' ? `app-dropdown-${props.width}` : '',
  position.value.above ? 'app-dropdown-above' : '',
])
...
function triggerFromEvent(event: Event) {
  ...
  return target?.closest<HTMLElement>('button, summary, [role="button"]') ?? event.currentTarget as HTMLElement
}
...
function close(focusAnchor = false) {
  ...
  setOpen(false)
  anchor.value = null
  if (focusAnchor) trigger?.focus()
}

function updatePosition() {
  if (!isOpen.value || !anchor.value || !panel.value) return
  const margin = 12
  const gap = 8
  ...
  const maxHeight = Math.max(80, Math.min(desiredMaxHeight, above ? availableAbove : availableBelow))
  const top = above
  ...
  position.value = { top, left, maxHeight, above }
}
...
watch(() => props.open, (open) => {
  ...
  else if (open === false) anchor.value = null
})
...
<template>
  ...
  >
    ...
    <div
      ...
      :class="[panelClass, props.panelClass]"
      :style="panelStyle"
      :role="panelRole"
      :aria-label="label"
    >
