...
import type { NewPatternProject, PrintMode } from '../types/pattern'
...
const { downloadCanvasPng } = usePatternExport({
  project: () => pattern.project.value,
  renderedPattern,
  includeAnnotations,
  onSuccess: () => notify(t('editor.notifications.pngDownloaded'), 'success'),
  onError: () => notify(t('editor.errors.pngFailed'), 'error'),
})
